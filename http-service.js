...
exports.attach = function () {
   var app = this;
   app.use(require('./servers.js'), app.config.get('servers'));
   //
   // Probes
   //
...
exports.init = function () {
   ...
   function handleAuthorized(req, res, cleanup) {
      ...
      var url = req.url;
      var handledByAuthorized = app.routers.authorized.dispatch(req, res);
      if (handledByAuthorized
      || (!app.config.get('authorization:no-fallthrough' && app.routers.unauthorized.dispatch(req, res)))) return;
      res.writeHead(404);
      res.end();
   }
